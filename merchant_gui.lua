...
local function autoPlantLoop()
    while autoPlant do
        for seedName, enabled in pairs(seedToggles) do
            if enabled then
                pcall(function()
                    -- Here, replace with the actual planting event call:
                    local position = LocalPlayer.Character.HumanoidRootPart.Position
                    local plantPos = Vector3.new(position.X, position.Y - 1, position.Z)  -- or wherever you want to plant
...
-- Now add autoPlant toggle here

local autoPlantToggle = createToggleButton(MerchantsTabFrame, "Auto Plant", 120)
    ...
    if autoPlantEnabled then
        task.spawn(function()
            while autoPlantEnabled do
                autoPlant("Tomato") -- Change to whatever seed you want
                task.wait(2)
            end
        end)
    end
